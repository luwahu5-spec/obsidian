...
- **Two-level model**: Customer record → IdP alias + tenant ID; User → `AuthenticationMethod` (Local|EnterpriseSSO) + `CustomerId`, CORE DB master (no KC attribute mirror needed for routing — see below). SSO only allowed if customer has an IdP configured.
- **Three enforcement layers that are code** (G4 is structural, not code — see superseded note below): (1) no KC password credential / no UPDATE_PASSWORD for SSO users (`KcClient.MigrateUser`, `KcClient.cs:280-295`); (2) KC conditional Deny (`Condition - user attribute` + `Deny Access`) on the **Reset Credentials** flow — built and verified 2026-09-09 — the forgot-password bypass is the one designs forget; (3) CORE hides/refuses password endpoints and locks the email field for SSO users.

> **⚠️ SUPERSEDED 2026-09-01/09-09 — the four bullets below (method switching, re-bind, "still open," bonus) describe an earlier design that was later dropped entirely.** Kept for history; do not implement. Current design: **no conversion in either direction is supported** — existing users' emails don't match their Entra identities, so Local→SSO is structurally impossible, and SSO users are simply created as SSO from day one. There is therefore no "Convert to SSO/Local" action and no "Reset identity link" re-bind action. The `tid`/`oid` values are still captured (`ExpectedTid`/`ExpectedOid`/`OidCapturedAtUtc` on the user) but are **reserved for future provisioning, not enforced at login** — each customer's Keycloak provider already accepts only its own tenant's tokens, and the pre-existing-user requirement (link-or-reject flow) already guarantees the right person, so a runtime CORE-side comparison was judged redundant. Current ticket breakdown: [[2026-08-27 SSO Enterprise Auth Ticket Breakdown]].
...
- ~~**Still open**: mismatch UX wording; audit event catalogue (NFR-A 5); offboarding latency = session lifetime; KC 20 shared login page until upgrade (interim: identity-first Username Form flow possible on KC 20).~~ (Login topology resolved below; audit catalogue dropped — see ticket doc.)
...
- 2026-09-09 — "Update our md file" (the ticket breakdown) after the design was finalized and approved. → Superseded the earlier conversion/re-bind/tid-enforcement design in this note: no conversion supported at all (structurally impossible), G4 is now a structural guarantee (per-customer provider binding + pre-existing user) rather than a runtime CORE check, `CustomerIdentityProvider` table dropped in favour of three columns on `Customer`. See [[2026-08-27 SSO Enterprise Auth Ticket Breakdown]] for the current ticket list.
